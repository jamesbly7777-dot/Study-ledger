...
function updateTimerBoxState() {
  ...
  if (el.timerBox)     el.timerBox.classList.toggle("running", isRunning),
                       el.timerBox.classList.toggle("paused",  isPaused);
  if (el.timerDisplay) el.timerDisplay.classList.toggle("running", isRunning),
                       el.timerDisplay.classList.toggle("paused",  isPaused);
  if (el.timerStatus)  el.timerStatus.classList.toggle("running", isRunning),
...
function getStreak() {
  ...
  while (true) {
    ...
    if (dayKeys.has(key)) { streak++; d.setDate(d.getDate() - 1); }
    else break;
  }
  return streak;
}
...
function renderSnapshot() {
  ...
  if (el.sessionCount) {
    ...
    el.sessionCount.textContent = `${n} session${n === 1 ? "" : "s"}`;
  }
}
...
function renderSessions() {
  ...
  el.sessionList.innerHTML = state.sessions.slice(0, 25).map(s => {
...
function renderChart() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const hours = state.sessions
      .filter(s => { const t = new Date(s.startedAt); return t >= dayStart && t < dayEnd; })
      .reduce((sum, s) => sum + (s.hours || 0), 0);
    days.push({ label: DAY_LABELS[dayStart.getDay()], hours, isToday: i === 0 });
  }

  const maxHours = Math.max(...days.map(d => d.hours), state.dailyGoal, 0.5);
  el.hoursChart.innerHTML = days.map(day => {
    const pct    = Math.min((day.hours / maxHours) * 100, 100);
    const height = Math.max(pct, 2);
    const label  = day.hours > 0 ? day.hours.toFixed(1) : "";
    return `
      <div class="bar-col">
...
function exportJson() {
  const payload = { sessions: state.sessions, dailyGoal: state.dailyGoal, exportedAt: new Date().toISOString() };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement("a");
  a.href = url; a.download = "atlas-study-backup.json"; a.click();
...
function importJson(file) {
  ...
  reader.onload = async () => {
    try {
      ...
      state.sessions  = Array.isArray(parsed.sessions) ? parsed.sessions : [];
      state.dailyGoal = typeof parsed.dailyGoal === "number" && parsed.dailyGoal > 0 ? parsed.dailyGoal : 2;
      saveLocalState();
      renderAll();
...
function addDemoData() {
  const now = new Date();
  state.sessions = [
    { id: String(Date.now())+"-1", title:"Google IT Networking", category:"Google IT",   focus:"4", notes:"Reviewed TCP vs UDP",            ms:5400000, hours:1.5,  startedAt:new Date(now-86400000*1).toISOString(), endedAt:new Date(now-86400000*1+5400000).toISOString() },
    { id: String(Date.now())+"-2", title:"Python Basics",        category:"Codecademy", focus:"5", notes:"Lists, dicts, functions",         ms:3600000, hours:1.0,  startedAt:new Date(now-86400000*2).toISOString(), endedAt:new Date(now-86400000*2+3600000).toISOString() },
    { id: String(Date.now())+"-3", title:"SQL Fundamentals",     category:"Database",   focus:"3", notes:"JOINs and subqueries",            ms:7200000, hours:2.0,  startedAt:new Date(now-86400000*4).toISOString(), endedAt:new Date(now-86400000*4+7200000).toISOString() },
    { id: String(Date.now())+"-4", title:"React Hooks Deep Dive",category:"Frontend",   focus:"5", notes:"useEffect, useCallback, useMemo", ms:4500000, hours:1.25, startedAt:new Date(now-86400000*5).toISOString(), endedAt:new Date(now-86400000*5+4500000).toISOString() }
  ];
  saveLocalState();
  renderAll();
...
function updateAuthUI() {
  ...
  [el.emailInput, el.passwordInput, el.signupBtn, el.loginBtn].forEach(node => {
    if (node) node.style.display = loggedIn ? "none" : "";
  });
  [el.logoutBtn, el.syncBtn].forEach(node => {
    if (node) node.style.display = loggedIn ? "" : "none";
  });
  if (el.authText) {
...
async function pushLocalToCloud(showSuccessToast = false) {
  ...
  try {
    ...
    console.error(err);
    if (showSuccessToast) showToast("Sync failed — check Firestore rules");
  }
}
...
async function loadFromCloud(showSuccessToast = false) {
  ...
  try {
    ...
    if (!doc.exists) {
      // New user — push any existing local data up so it isn't lost
    ...
    if (typeof data.dailyGoal === "number" && data.dailyGoal > 0) state.dailyGoal = data.dailyGoal;
    saveLocalState();
    renderAll();
